...
module.exports = class IO {
  ...
  async destroy () {
    ...
    console.log('beginning io destroy')
    this._destroying = this._destroy()
    const res = await this._destroying
    console.log('io done destroying')
    return res
  }

  async _destroy () {
    // simplifies timing to await the bind here also, although it might be unneeded
    console.log('io awaiting bind')
    await this.bind()
    console.log('io awaiting _clear')
    await this._clear(false)
    console.log('io _destroy done')
  }
  ...
  async _clear (suspended) {
    ...
    console.log(this.counter, new Date(), 'io_clear is awaiting socket closing')
    await Promise.allSettled([
      this.serverSocket.close(),
      this.clientSocket.close()
    ])
    console.log(this.counter, new Date(), 'io sockets closed')
